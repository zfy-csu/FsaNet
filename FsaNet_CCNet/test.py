...
BATCH_SIZE = 1
NUM_CLASSES = 19
TEST_STEPS = 1531 #default Number of images in the test set, make it small can stop earlier.
INPUT_SIZE = '769,769'
RESTORE_FROM = './snapshots/City25ft_2500.pth' # './snapshots/11_2nla_10000.pth'
...
def get_parser():
    ...
    """
    parser = argparse.ArgumentParser(description="Fsanet Network")
    ...
    parser.add_argument("--Output", type=str, default='./snapshots/', #zfy
                        help="save path.")
    parser.add_argument("--gpu", type=str, default='0',
                        help="choose gpu device.")
    parser.add_argument("--num-workers", type=int, default=0,
                        help="choose the number of works.")
    parser.add_argument("--whole", type=bool, default=False,
                        help="use whole input size.")
    parser.add_argument("--num-steps", type=int, default=TEST_STEPS,#zfy
    ...
    parser.add_argument("--experimentID", type=str, default='r2CityDot',
                        help="use for result file.")
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        print('test initmodel:',args.restore_from)
        print('test batch_size:', args.batch_size)
        print('test inputsize:',args.input_size)
        print('test model:',args.model)
        print('test att_mode:',args.att_mode)
        print('test k:',args.k) 
        print('test inputsize:',args.input_size)
        print('test data_list:',args.data_list)
        print('test recurrence:',args.recurrence)
        print('test scale_list:',args.scale_list)
        print('test experimentID',args.experimentID)
        ...
        dataset = CSDataTestSet(args.data_dir, args.data_list, crop_size=(1024, 2048), mean=IMG_MEAN)
        test_loader, test_sampler = engine.get_test_loader(dataset)

        if engine.distributed:
            test_sampler.set_epoch(0)

        save_path = os.path.join(os.path.dirname(args.Output), 'outputs'+args.experimentID)
        ...
        for idx in pbar:
            image, size, name = dataloader.next()
            size = size[0].numpy()
            with torch.no_grad():
            ...
            offi_seg_pred = dataset.id2trainId(seg_pred,reverse=True) #official evaluation
            for i in range(image.size(0)): #zfy                 
                output_im = PILImage.fromarray(offi_seg_pred[i]) #zfy
                output_im.save(os.path.join(save_path, name[i]+'.png'))

            print_str = ' Iter{}/{}'.format(idx + 1, len(test_loader))
            pbar.set_description(print_str, refresh=False)
            
            if idx >= args.num_steps: #early stop
                break
